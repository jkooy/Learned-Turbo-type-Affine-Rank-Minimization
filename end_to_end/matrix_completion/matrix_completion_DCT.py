...
logfile = './307end_end_logger_DCT.txt'
...
model.load_state_dict(torch.load('307end_to_end_DCT.pkl'))
...
for t in range(epoch_num):
    ...
    for ii in range(train_batch):
        L = matrix_generate(M, N, r, "DCT matrix")
    ...
    if loss_dB <= min(losslist):
        nmsebest = loss_dB
        torch.save(model.state_dict(), str(layer_num_) + '307end_to_end_DCT.pkl')
        nmsebestlist.append(loss_dB)


"test set"
test_num= 1000
model.load_state_dict(torch.load(str(layer_num_)+'307end_to_end_DCT.pkl'))
