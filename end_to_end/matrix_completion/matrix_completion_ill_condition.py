...
logfile = './307_end_to_end_ill_condition.txt'
...
model.load_state_dict(torch.load('30307end_to_end_ill_condition.pkl'))
...
for t in range(epoch_num):
    ...
    for ii in range(train_batch):
        L = matrix_generate(M, N, r, "ill_conditioned matrix")
    ...
    if loss_dB <= min(losslist):
        nmsebest = loss_dB
        torch.save(model.state_dict(), str(layer_num_) + '307end_to_end_ill_condition.pkl')
        nmsebestlist.append(loss_dB)


"test set"
test_batch= 100
model.load_state_dict(torch.load(str(layer_num_)+'307end_to_end_ill_condition.pkl'))
...
for ii in range(test_batch):
    ...
    for layer in range(layer_num_):
        loss = criterion(L_layer[layer], L) / (L.pow(2).sum())
...
for t in range(test_num):
    ...
    y_pred = model(L, r)
    for layer in range(layer_num_):
        loss = criterion(y_pred[layer], L) / (L.pow(2).sum())
        losslayerlist.append(10 * torch.log10(loss.data))
        layerlist.append((layer + 1))
