...
% in_params.r=r;
 ...
 perm=randperm(n);
 indexs=perm(1:m);
 indexs=cell2mat(struct2cell(load('indexss.mat')));
...
for ii=1:total_batch
    ...
    for jj=1:train_num
        ...
        L=sqrt(n)*L/norm(L,'fro');
        L=cell2mat(struct2cell(load('LL.mat')));
        sigma=0;
        sigma=sqrt(10^(-5));
        Ln=L+sigma*randn(n1,n2);
        Ln=cell2mat(struct2cell(load('L.mat')));
        ...
        % forward passing
          for lay=1:layer_num
             if lay==1
                ...
                dd=y-A(Lo);
                G=At(y-A(Lo));
                [Loa2,cachea2(lay)]=LMR_forward(Loa2,param_a2(lay),param_u1(lay),param_c1(lay),param_lamda1(lay),y,A,At);
                [Loc2,cachec2(lay)]=LMR_forward(Loc2,param_a1(lay),param_u1(lay),param_c2(lay),param_lamda1(lay),y,A,At);
                [Lou2,cacheau(lay)]=LMR_forward(Lou2,param_a1(lay),param_u2(lay),param_c1(lay),param_lamda1(lay),y,A,At);
                [Lolamda2,cachelamda2(lay)]=LMR_forward(Lolamda2,param_a1(lay),param_u1(lay),param_c1(lay),param_lamda2(lay),y,A,At);
                [Lo,cache(lay)]=LMR_forward(Lo,param_a1(lay),param_u1(lay),param_c1(lay),param_lamda1(lay),y,A,At);
  ...
  % test set
          ...
          for lay=1:layer_num
             if lay==1
                ...
                [layer_lo,cache(lay)]=LMR_forward(layer_lo,param_a1_best(lay),param_u1_best(lay),param_c1_best(lay),param_lamda1_best(lay),y,A,At);
             end
