...
import RPCA_matrix_recovery_network
...
model = nn.Sequential()
model.add_module('layer0',RPCA_matrix_recovery_network.input_layer(M, N,  indexs))
...
for layer_num in range(layer_num_):
    if layer_num>=1:
       ...
       model.add_module('layer'+str(layer_num),RPCA_matrix_recovery_network.hidden_layer(M, N, indexs, u_lay, a_lay, c_lay, u_lay2, a_lay2, c_lay2, lamda_lay2))
       #
    ...
    for t in range(300):
        ...
        if loss_dB<=min(losslist):
            nmsebest = loss_dB
            torch.save(model.state_dict(), str(layer_num)+'layerwiseRPCArecovery_paramsGPUr20.pkl')
            nmsebestlist.append(loss_dB)




model.load_state_dict(torch.load(str(layer_num)+'layerwiseRPCArecovery_paramsGPUr20.pkl'))
...
for layer in range(1,len(model)):
    ...
    L_pred, S_pred, GA, y_input, r_input = removed([y, r, GA])
    loss_L = criterion(L_pred, L)/(L.pow(2).sum())
    loss_S = criterion(S_pred, S) / ((S).pow(2).sum())
