...
import layer_by_layer_matrix_recovery_network
...
logfile = './matrix_recoverycosine.txt'
...
model = nn.Sequential()
model.add_module('layer0', layer_by_layer_matrix_recovery_network.input_layer(M, N, indexs))

"import trained parameters"
# for layer_num in range(1, 30):
#     model.add_module('layer'+str(layer_num),layer_by_layer_matrix_recovery_network.hidden_layer(M, N, indexs, torch.Tensor([[1]]), torch.Tensor([[1]]), torch.Tensor([[1]])))
# model.load_state_dict(torch.load('30general_matrix_cosine_recoveryr20.pkl'))
...
for layer_num in range(0, layer_num_):
    if layer_num > 1:
       ...
       model.add_module('layer'+str(layer_num),layer_by_layer_matrix_recovery_network.hidden_layer(M, N, indexs, u_lay, a_lay, c_lay))
    ...
    for t in range(epoch_num):
        ...
        if loss_dB<=min(losslist):
            nmsebest = loss_dB
            torch.save(model.state_dict(), str(layer_num_)+'general_matrix_cosine_recoveryr20.pkl')
...
"test set"
test_num= 1000
model.load_state_dict(torch.load(str(layer_num_)+'general_matrix_cosine_recoveryr20.pkl'))
