...
for layer_num in range(0, layer_num_):
    ...
    for t in range(iteration_num):
        ...
        if loss_dB<=min(losslist):
            nmsebest = loss_dB
            torch.save(model.state_dict(), str(layer_num_)+'trainA_matrix_cosine_recoveryr20.pkl')
            nmsebestlist.append(loss_dB)




model.load_state_dict(torch.load(str(layer_num_)+'trainA_matrix_cosine_recoveryr20.pkl'))
